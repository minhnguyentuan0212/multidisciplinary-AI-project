...
async function getAllRoomsData() {
  ...
  let res = data.data.map((item) => ({
    ...item,
    icon: IconMapping[item.name.toLowerCase()],
  }));
  let result = [];
    ...
    datasets: [
      {
        ...
        fill: false,
        pointRadius: 0,
        borderColor: data.key.includes("temp") ? "Orange" : "Blue",
      },
    ],
  };
...
async function getDevicesOfRoom(room_id) {
  ...
  for (var i in data.data) {
    ...
    if (device.key.includes("humid") || device.key.includes("temp")) {
      const signal = await processSignalData(device);
      devices.signal.push(signal);
    } else devices.devices.push(device);
  }
  return devices;
}
